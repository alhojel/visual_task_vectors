...
def evaluate(args):
    ...
    decoder_group = [decoder_ranked_task] #[decoder_ranked_task[:a] for a in range(decoder_start, decoder_end,decoder_step)] #+ [decoder_ranked_neutral[:a] for a in range(decoder_start, decoder_end,10)]
    
    encoder_group = [encoder_ranked_task] #[encoder_ranked_task[:a] for a in range(encoder_start, encoder_end,encoder_step)] #+ [encoder_ranked_neutral[:a] for a in range(encoder_start, encoder_end,10)] + [encoder_mid]
