...
def get_args():
    ...
    parser.add_argument('--section', default="encoder", type=str)
    parser.add_argument('--vector_type', default="avg", type=str)
...
def _generate_result_for_canvas(args, model, canvas, premask_pass_indices = None, postmask_pass_indices = None, attention_heads=None, attention_injection=None, replace=1, abalate=False, a_e_attention_injection=None, a_d_attention_injection=None):
    """canvas is already in the right range."""

    if a_e_attention_injection is None:
        ...
        ids_shuffle, len_keep = generate_mask_for_evaluation()
        _, im_paste, _, latents = generate_image(canvas.unsqueeze(0).to(args.device), model, ids_shuffle.to(args.device),
                                    len_keep, device=args.device, attention_heads = attention_heads, replace=replace, abalate=abalate, a_e_attention_injection=a_e_attention_injection.to(args.device), a_d_attention_injection=a_d_attention_injection.to(args.device))
...
def evaluate(args):
    ...
    for idx in trange(len(ds)):
        ...
        for i in range(len(canvas)):
            ...
            d_injection = torch.tensor(data["decoder"][captions[i]][vector_type])            
            e_injection = torch.tensor(data["encoder"][captions[i]][vector_type])        

            if args.section == "encoder":
                layer_range = range(24)
            else:
                layer_range = range(24, 32)
            
            for layer in layer_range:
                
                for head in range(16):
                    a_e_attention_injection = 0.3*e_injection
                    a_d_attention_injection = 0.3*d_injection

                    if args.section == "encoder":
                        a_e_attention_injection[layer, :, head] = 0
                    else:
                        a_d_attention_injection[layer-24, :, head] = 0

                    og2, gen2, latents = _generate_result_for_canvas(args, model, curr_canvas, a_e_attention_injection=a_e_attention_injection, a_d_attention_injection=a_d_attention_injection, replace = 0 if vector_type=="avg" else 1, abalate=True)
            ...
            if args.output_dir and args.save_images is not None and idx % args.save_images == 0:
                    ...
                    num_rows = len(layer_range)  # Select num_rows so that all the images fit
                    ...
                    plt.savefig(os.path.join(args.output_dir, f'combined_{idx}_{captions[i]}_{args.section}_{vector_type}.png'))
                    plt.show() 
