...
def get_args():
    ...
    parser.add_argument('--vector_type', default="avg_delta", type=str)
...
def evaluate(args):
    ...
    for idx in trange(len(ds)):
        ...
        for i in range(len(canvas)):
            ...
            for coeff in [0,0.1,0.3,0.5,0.7,0.9]:
                a_e_attention_injection = coeff*e_injection
                a_d_attention_injection = coeff*d_injection

                og2, gen2, latents = _generate_result_for_canvas(args, model, curr_canvas, a_e_attention_injection=a_e_attention_injection, a_d_attention_injection=a_d_attention_injection, replace = 1, abalate=True)
                ...
                label_holder.append(f"Coeff {coeff}")
            ...
            if args.output_dir and args.save_images is not None and idx % args.save_images == 0:
                    ...
                    num_rows =  6 # Select num_rows so that all the images fit
                    num_cols = 2  # Select num_rows so that all the images fit
