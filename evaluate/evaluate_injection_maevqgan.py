...
def _generate_result_for_canvas(args, model, canvas, encoder_task_vector=None, decoder_task_vector=None, convex="False"):
    ...
    if decoder_task_vector is not None:
        ...
        _, im_paste, _, latents = generate_image(canvas.unsqueeze(0).to(args.device), model, ids_shuffle.to(args.device),
                                    len_keep = len_keep, e_vec = None, d_vec = None, device=args.device, convex=False)
...
def evaluate(args):
    ...
    for idx in trange(len(ds)):
        ...
        for i in range(len(canvas)):
            ...
            decoder_task_vector = torch.tensor(data[vector_model[1]]["decoder"])
            ...
            #coeff_array = np.append(np.array(0), np.arange(1,2,0.1))
            coeff_array = np.arange(1,3,0.1)

            for tokens in ["diagonal_down", "diagonal_up"]:
                for coeff_first in coeff_array:
                    
                #for tokens in ["all",  "top_half", "bottom_half","left_half", "right_half", "except_query",  "right_half", "top_left", "top_right", "bottom_left", "bottom_right"]:
                

                    zero_tensor = torch.zeros_like(decoder_task_vector)
                    if tokens == "all":
                        zero_tensor[0] = decoder_task_vector[0]

                    if tokens == "left_half":
                        zero_tensor[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 1)]] = decoder_task_vector[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 1)]]
                        zero_tensor[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 3)]] = decoder_task_vector[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 3)]]
                        
                    if tokens == "right_half":
                        zero_tensor[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 2)]] = decoder_task_vector[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 2)]]
                        zero_tensor[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 4)]] = decoder_task_vector[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 4)]]
                    
                    if tokens == "bottom_half":
                        zero_tensor[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 3)]] = decoder_task_vector[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 3)]]
                        zero_tensor[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 4)]] = decoder_task_vector[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 4)]]
                    ...
                    if tokens == "except_query":
                        zero_tensor[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 1)]] = decoder_task_vector[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 1)]]
                        zero_tensor[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 2)]] = decoder_task_vector[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 2)]]
                        zero_tensor[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 4)]] = decoder_task_vector[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 4)]]
                    
                    if tokens == "except_top_left":
                        zero_tensor[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 3)]] = decoder_task_vector[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 3)]]
                        zero_tensor[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 2)]] = decoder_task_vector[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 2)]]
                        zero_tensor[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 4)]] = decoder_task_vector[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 4)]]
                    if tokens == "except_top_right":
                        zero_tensor[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 3)]] = decoder_task_vector[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 3)]]
                        zero_tensor[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 1)]] = decoder_task_vector[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 1)]]
                        zero_tensor[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 4)]] = decoder_task_vector[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 4)]]
                    if tokens == "except_bottom_right":
                        zero_tensor[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 3)]] = decoder_task_vector[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 3)]]
                        zero_tensor[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 1)]] = decoder_task_vector[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 1)]]
                        zero_tensor[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 2)]] = decoder_task_vector[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 2)]]
                    
                    if tokens == "diagonal_down":
                        zero_tensor[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 1)]] = decoder_task_vector[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 1)]]
                        zero_tensor[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 4)]] = decoder_task_vector[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 4)]]
                    if tokens == "diagonal_up":
                        zero_tensor[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 3)]] = decoder_task_vector[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 3)]]
                        zero_tensor[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 2)]] = decoder_task_vector[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 2)]]
                    ...
                    if tokens == "bottom_left":
                        zero_tensor[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 3)]] = decoder_task_vector[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 3)]]
                    if tokens == "bottom_right":
                        zero_tensor[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 4)]] = decoder_task_vector[0][:,[1+a for a in calculate_quadrant_indices(14, 14, 4)]]

                    zero_tensor[0, :, 0] = decoder_task_vector[0, :, 0]

                    original_image, generated_result, latents = _generate_result_for_canvas(args, model, curr_canvas,decoder_task_vector=coeff_first*zero_tensor, convex="False")
