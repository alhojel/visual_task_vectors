...
def get_args():
    ...
    parser.add_argument('--image_suffix', default=1, type=float)
...
def evaluate(args):
    ...
    labels = ["None", "23-13-CLS", "5-4-CLS", "5-14-Q", "6-0-Q", "10-12-Q", "21-15-Q", "25-2-Q", "25-2-M", "30-7-Q", "30-7-M"]#,"25-0-M"]#, "0-6-M"]
    ...
    for idx in trange(len(ds)):
        ...
        for i in range(len(canvas)):
            ...
            for vector_type in ["segmentation"]:#,"segmentation_1","segmentation_2","segmentation_3"]:

               
                enc_inj = args.image_suffix*torch.tensor(injection_master["encoder"][vector_type]).to(args.device)
                dec_inj = args.image_suffix*torch.tensor(injection_master["decoder"][vector_type]).to(args.device)
                ...
                for index in range(len(target_array)):
                    ...
                    if target_array[index][0] is None:
                        ...
                        og2, gen2, latents = _generate_result_for_canvas(args, model, curr_canvas, premask_pass_indices = indices_premask, attention_heads=target_array[index][0], attention_injection = injection, postmask_pass_indices = indices_postmask, drop_indices = drop_indices)#
                    ...
                    with open(os.path.join(args.output_dir, 'log.txt'), 'a') as log:
                        ...
                        current_metric["targets"] = labels[index]
                        current_metric["vector_type"] = str(args.image_suffix)+vector_type
                ...
                if args.output_dir and args.save_images is not None and idx % args.save_images == 0:
                    ...
                    num_cols = 3  # Number of columns can be adjusted as needed
                    ...
                    plt.savefig(os.path.join(args.output_dir, f'combined_{idx}_{vector_type}_{args.image_suffix}.png'))
                    plt.show() 
