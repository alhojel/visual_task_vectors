...
ordered_layer_list_enconly =  [3, 23, 22,  0, 13, 21, 10, 16,  5, 11, 4,  2, 20,  7,  9, 12,  8,  6, 15, 19,  1, 18, 17, 14]
ordered_layer_list_deconly =  [30, 25, 31, 28, 27, 29, 24, 26]
...
class JointModel(nn.Module):
    def __init__(self, args, prompting_model, num_variables, train_ds, eval_ds, task_tensor, load_model=None):
        ...
        self.bernoullis =[]#[[25,2,2], [26,0,2], [10,12,1], [16,13,1], [25,10,1], [25,10,2]]
    ...
    def get_good_init(self, args, canvases, sample_count, all_areas):
        ...
        for sample in range(sample_count):
            ...
            rand_select = [area for area in all_areas if random.random() < 0.15]
    ...
    def train(self, args, num_itr):
        ...
        start_layers = ordered_layer_list[:args.k]
        start_quadrants = []
        ...
        if args.granularity==1:
            if args.zero_shot:
                ...
                for layer in start_layers:
                    if layer<=23:
                        start_quadrants += [[layer, head, quadrant] for head in range(0,16) for quadrant in [0,1,2,3]]
                    if layer>=24:
                        start_quadrants += [[layer, head, quadrant] for head in range(0,16) for quadrant in [0,1,2,3,4]]


        if args.granularity==2:
            for layer in start_layers:
                start_quadrants += [[layer, head] for head in range(0,16)]
                
        if args.granularity==3:
            for layer in start_layers:
                start_quadrants += [[layer]]    
        
        init_loss, _ = self.get_good_init(args, canvases, 100, start_quadrants)
        ...
        for i in trange(num_itr):
            for layer_idx in range(len(start_layers)):
                layer = start_layers[layer_idx]
                if args.granularity==1:
                    if layer<=23:
                        self.areas_to_check = [[layer, head, quadrant] for head in range(0,16) for quadrant in [0,1]] + [None]
                    if layer>=24:
                        self.areas_to_check = [[layer, head, quadrant] for head in range(0,16) for quadrant in [0,1,2]]+ [None]
                if args.granularity==2:
                    self.areas_to_check = [[layer, head] for head in range(0,16)] + [None]

                if args.granularity==3:
                    self.areas_to_check = [[layer]] + [None]
                
                best_loss, res, move = self.forward(args, canvases, i)

                print("Layer:",layer_idx,"Loss:",best_loss, "Bernoullis:", len(self.bernoullis), "Move:", move)
...
def evaluate(args):
    ...
    eval_ds = rl_dataloader.DatasetPASCAL(args.base_dir, fold=args.split+2, image_transform=image_transform, mask_transform=mask_transform,
                         flipped_order=args.flip, purple=args.purple, query_support_list_file=args.query_support_list_file, iters=args.eval_iters, type="trn", task=args.task)
