...
class JointModel(nn.Module):
    def __init__(self, args, prompting_model, num_variables, train_ds, eval_ds, task_tensor, load_model=None):
        ...
        if args.restrict_area==1:
            for i in range(24*16*2):
                self.bernoullis[i] = torch.tensor(0.0, requires_grad=True)
    ...
    def train(self, args, num_itr):
        ...
        for i in trange(num_itr):
            ...
            loss, res, iou = self.forward(args, canvases)
            loss.backward()
            self.optim.step()

            if i % 10 == 0:
                print("IoU:",i,iou)
                image = res.detach().cpu().numpy()
                plt.figure()
                plt.imshow(image)
                plt.axis('off')  # Turn off axis numbers and ticks
                image_save_path = os.path.join(args.output_dir, f'loop_{args.task}_{args.granularity}_{self.regularization_strength}_{args.train_images}_{args.lr}_{args.restrict_area}_{args.init}_{idx}_{i}.png')
                plt.savefig(image_save_path, bbox_inches='tight', pad_inches=0)  # Save the image without padding and no axis
                plt.close()  # Close the plot to prevent it from displaying in the notebook or script output
                ...
                with open(os.path.join(args.output_dir,'log.txt'), 'a') as log:
                    ...
                    current_metric["train_loss"] = iou.item()
...
def evaluate(args):
    ...
    eval_ds = rl_dataloader.DatasetPASCAL(args.base_dir, fold=args.split+2, image_transform=image_transform, mask_transform=mask_transform,
                         flipped_order=args.flip, purple=args.purple, query_support_list_file=args.query_support_list_file, iters=args.eval_iters, type="trn", task= args.task if args.task is not None else 0)
    ...
    rl_model.train(args, num_itr=600)
