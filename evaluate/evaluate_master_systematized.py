...
def evaluate(args):
    ...
    baseline = []
    baseline.extend(component_lists)

    def get_unique(master):
        curr_array = []
        for sss in master:
            curr_array.extend(sss)

        unique_tuples = set(tuple(x) for x in curr_array)

        # Convert each tuple back into a list
        unique_lists = [list(x) for x in unique_tuples]

        return unique_lists #curr_array
    
    target_array = [[get_unique(baseline + encoder_group[a] + decoder_group[b])] for a in range(len(encoder_group)) for b in range(len(decoder_group))]

    
    #target_array = [[None], 
                    # [[[23,13,0]]],
                    # [[[23,13,0]]+[[25, 2, a+1] for a in calculate_quadrant_indices(14, 14, 2)]],
                    # [[[23,13,0]]+ [[25, 2, a+1] for a in calculate_quadrant_indices(14, 14, 2)]+[[25, 10, a+1] for a in calculate_quadrant_indices(14, 14, 2)]],
                    # [[[23,13,0]]+ [[25, 2, a+1] for a in calculate_quadrant_indices(14, 14, 2)]+[[25, 10, a+1] for a in calculate_quadrant_indices(14, 14, 2)]+[[25, 7, a+1] for a in calculate_quadrant_indices(14, 14, 2)]],
                    # [[[23,13,0]]+ [[25, 2, a+1] for a in calculate_quadrant_indices(14, 14, 2)]+[[25, 10, a+1] for a in calculate_quadrant_indices(14, 14, 2)]+[[25, 7, a+1] for a in calculate_quadrant_indices(14, 14, 2)]+[[24, 13, a+1] for a in calculate_quadrant_indices(14, 14, 2)]],
                    # [[[23,13,0]]+ [[25, 2, a+1] for a in calculate_quadrant_indices(14, 14, 2)]+[[25, 10, a+1] for a in calculate_quadrant_indices(14, 14, 2)]+[[25, 7, a+1] for a in calculate_quadrant_indices(14, 14, 2)]+[[24, 13, a+1] for a in calculate_quadrant_indices(14, 14, 2)]]+[[23, 13, a+1] for a in calculate_quadrant_indices(14, 14, 2)],
                    # [[[23,13,0]]+ [[22,14,0]]+ [[25, 2, a+1] for a in calculate_quadrant_indices(14, 14, 2)]+[[25, 10, a+1] for a in calculate_quadrant_indices(14, 14, 2)]+[[25, 7, a+1] for a in calculate_quadrant_indices(14, 14, 2)]+[[24, 13, a+1] for a in calculate_quadrant_indices(14, 14, 2)]]+[[23, 13, a+1] for a in calculate_quadrant_indices(14, 14, 2)]+[[23, 13, a+1] for a in calculate_quadrant_indices(14, 14, 2)],
                    # [[[23,13,0]]+ [[22,14,0]]+ [[25, 2, a+1] for a in calculate_quadrant_indices(14, 14, 2)]+[[25, 10, a+1] for a in calculate_quadrant_indices(14, 14, 2)]+[[25, 7, a+1] for a in calculate_quadrant_indices(14, 14, 2)]+[[24, 13, a+1] for a in calculate_quadrant_indices(14, 14, 2)]]+[[23, 13, a+1] for a in calculate_quadrant_indices(14, 14, 1)]+[[23, 13, a+1] for a in calculate_quadrant_indices(14, 14, 2)]+[[22, 14, a+1] for a in calculate_quadrant_indices(14, 14, 1)],
                    # [[[23,13,0]]+ [[21,12,0]]+ [[0,0,0]]+ [[15,11,0]]+ [[5,4,0]]+ [[11,5,0]]+ [[22,14,0]]+ [[25, 2, a+1] for a in calculate_quadrant_indices(14, 14, 2)]+[[25, 10, a+1] for a in calculate_quadrant_indices(14, 14, 2)]+[[25, 7, a+1] for a in calculate_quadrant_indices(14, 14, 2)]+[[24, 13, a+1] for a in calculate_quadrant_indices(14, 14, 2)]]+[[23, 13, a+1] for a in calculate_quadrant_indices(14, 14, 1)]+[[23, 13, a+1] for a in calculate_quadrant_indices(14, 14, 2)]+[[21, 2, a+1] for a in calculate_quadrant_indices(14, 14, 1)]+[[10, 2, a+1] for a in calculate_quadrant_indices(14, 14, 1)]+[[9, 8, a+1] for a in calculate_quadrant_indices(14, 14, 1)]+[[23, 8, a+1] for a in calculate_quadrant_indices(14, 14, 1)]+[[23, 14, a+1] for a in calculate_quadrant_indices(14, 14, 1)]+[[23, 5, a+1] for a in calculate_quadrant_indices(14, 14, 1)]+[[23, 15, a+1] for a in calculate_quadrant_indices(14, 14, 1)]+[[23, 15, a+1] for a in calculate_quadrant_indices(14, 14, 1)]]
    
    #labels = ["None", "+ 23-14-CLS", "+ 25-2-M", "+ 25-10-M", "+ 25-7-M", "+ 24-13-M", "+ 23-13-Q", "+ 22-14-CLS", "+ 24-14-Q", "+Many Enc"]#= ["e_"+str(a) for a in range(encoder_start, encoder_end,encoder_step)] #+ ["encoder_neutral_"+str(a) for a in range(encoder_start, encoder_end,10)] + ["encoder_mid"]
    ...
    for idx in trange(len(ds)):
        ...
        for i in range(len(canvas)):
            ...
            for vector_type in ["segmentation_neutral"]:#"segmentation_1","segmentation_2","segmentation_3"]:
                ...
                with open(os.path.join(args.output_dir, 'log.txt'), 'a') as log:
                    ...
                    current_metric["task"] = captions[i]+"_neutral"
                ...
                for encoder_index, decoder_index in itertools.product(range(len(encoder_group)), range(len(decoder_group))):
                    ...
                    if target_array[index][0] is None:
                        og2, gen2, latents = _generate_result_for_canvas(args, model, copy_canvas, premask_pass_indices = indices_premask, postmask_pass_indices = indices_postmask, drop_indices = drop_indices)#
                    else:
                    ...
                    gen_holder.append(gen2)
                    label_holder.append(decoder_labels[decoder_index]+"\n"+encoder_labels[encoder_index])
                ...
                if args.output_dir and args.save_images is not None and idx % args.save_images == 0:
                    ...
                    for r in range(1, num_rows):
                        for c in range(num_cols):
                            index = (r-1) * num_cols + c + 3 # Adjusting index to skip the first two images
                    ...
                    plt.savefig(os.path.join(args.output_dir, f'combined_{idx}_neutral_{vector_type}.png'))
                    plt.show() 
