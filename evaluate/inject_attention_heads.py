...
def _generate_result_for_canvas(args, model, canvas, premask_pass_indices = None, postmask_pass_indices = None, attention_heads=None, attention_injection=None, replace=1):
    """canvas is already in the right range."""

    if attention_injection is None:
        ...
        ids_shuffle, len_keep = generate_mask_for_evaluation()
        _, im_paste, _, latents = generate_image(canvas.unsqueeze(0).to(args.device), model, ids_shuffle.to(args.device),
                                    len_keep, device=args.device, attention_heads = attention_heads, attention_injection=attention_injection.to(args.device), replace=replace)
    ...
    for idx in trange(len(ds)):
        ...
        for i in range(len(canvas)):
            ...
            if args.section == "encoder":
                ...
                injection = torch.tensor(data["encoder"][captions[i]][vector_type])
            else:
                layer_range = range(24, 32)
                injection = torch.tensor(data["decoder"][captions[i]][vector_type])    
            ...
            for layer in layer_range:
                ...
                for head in range(16):
                    og2, gen2, latents = _generate_result_for_canvas(args, model, curr_canvas, attention_heads=[layer,head], attention_injection=injection, replace = 0 if vector_type=="avg" else 1)
